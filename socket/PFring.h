...
public:
	...
	int get_next_packet(struct pfring_pkthdr *hdr, char** pkt, u_int pkt_len, u_int8_t wait_for_incoming_packet);
	int start_loop(pfringProcesssPacket looper);

	/* Send Packets */
	int send_packet(char *pkt, u_int pkt_len, bool flush = true, bool active_poll = true);
	...
	/* Utils */
	inline char* get_device_name() {
		return (device_name);
	}

	inline int enable_ring() {
		return (ring ? pfring_enable_ring(ring) : -1);
	}
	...
	inline u_int8_t get_num_rx_channels() {
		return (ring ? pfring_get_num_rx_channels(ring) : -1);
	}
