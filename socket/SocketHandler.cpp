...
//		ioctl(socket_, SIOCSIFFLAGS, &ifopts);
	...
	int NetworkHandler::GetNextFrame(struct pfring_pkthdr *hdr, char** pkt,
		...
		int rc = 0;
		try {
			rc = recv(socket_, (void*) recvBuffer_, BUF_SIZE, 0);
			if (rc == -1) {
				LOG_INFO << "recieved error" << ENDL;
			}
		} catch(int e) {

			LOG_INFO << "TRY CATCH 'EM ALL!" << ENDL;
		}
		if (rc == -1) {
			LOG_INFO << "but made it further" << ENDL;
			return 0;
		}
		LOG_INFO << "recieved " << rc << " Bytes" << ENDL;
		*pkt = recvBuffer_;
		hdr->len = rc;
		framesReceived_++;
